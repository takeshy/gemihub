...
- **Sandboxed Execution**: Plugins run via `new Function` with a controlled `require` shim (react, react-dom only)
...
- No CLI/shell access
- No direct network requests to external APIs (use `api.gemini` for AI calls)
- `require()` only provides `react`, `react-dom`, and `react-dom/client`
