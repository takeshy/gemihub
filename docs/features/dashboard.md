...
- **Storage and synchronization** — each day is a Markdown file `Dashboards/Timeline/<name>/<YYYY-MM-DD>.md`; posts are `---`-separated blocks marked with a `<!-- timeline-post: … -->`-style header carrying the ISO timestamp, `id`, and optional `pinned: true`. Image attachments are saved as binary files under `…/attachments/<date>/<postId>_NN.<ext>` and embedded in the post body as Obsidian embeds (`![[…]]`). Online operations read the latest Drive file before applying the change and update Drive immediately. A checksum guard retries when another client changes the file concurrently. The header's reload button manually refreshes that Timeline from Drive. Offline writes remain local and migrate when connectivity returns.
...
- **Delete** — a trash icon on each row (visible on hover) deletes the secret via `deleteFileLocal` after a confirmation prompt; this trashes the underlying Drive file like other delete flows in the app.
- **Unlock and edit** — opening a secret uses the session-cached private key/password when available, otherwise it prompts for the encryption password. The unlocked value can be copied or edited and re-encrypted in place. Edit mode also has a Directory field (pre-filled with the secret's current directory) — changing it moves the secret on save, same as drag-and-drop; since directories are purely derived from the path, typing a directory that doesn't exist yet just creates it. An open-file icon in the detail modal's header navigates to the underlying `.encrypted` file itself (via the shared `plugin-select-file` event) and closes the modal.
