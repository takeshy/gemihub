...
Cloud Run (single, multi-tenant)
  ...
  ├── Hubwork API (account resolved from Host header)
...
1. Admin enters domain in Settings > Hubwork > Custom Domain
...
When Hubwork is enabled, a built-in Agent Skill ("Hubwork Web") is automatically provisioned to `skills/hubwork-web/` on Drive. This skill enables building web pages and API endpoints through AI chat.

### Auto-Provisioning

The skill is created on the first visit to the Settings page after Hubwork activation. It includes:
...
1. Open Chat and activate the "Hubwork Web" skill from the Skills dropdown
...
1. User clicks "Subscribe" in **Settings > Hubwork** tab
...
5. User returns to Settings — Hubwork features are automatically enabled (no separate toggle needed)
...
| GET | `/hubwork/admin` | OAuth email | Admin dashboard |
| GET/POST | `/hubwork/admin/accounts/:id` | OAuth email | Account detail/edit/delete |
| GET/POST | `/hubwork/admin/accounts/create` | OAuth email | Create granted account |
...
| `HUBWORK_ADMIN_EMAILS` | Comma-separated admin emails (default: `takesy.morito@gmail.com`) |
...
| `app/services/hubwork-admin-auth.server.ts` | Admin panel auth (OAuth email check) |
...
| `app/services/hubwork-skill-provisioner.server.ts` | Auto-provision "Hubwork Web" skill to Drive |
