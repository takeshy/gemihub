...
RAG タブには OKF（Open Knowledge Format）ナレッジソースの設定も含まれる。`okfRoot`（Drive ルートからの相対フォルダパス）を設定してバンドルを選択すると、選択バンドルの要約がチャットのシステムプロンプトに注入される（File Search への登録ではなくプロンプトコンテキストとしての注入）。詳細は [OKF_ja.md](./OKF_ja.md) を参照。
