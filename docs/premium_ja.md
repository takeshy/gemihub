...
- **2 層 URL** — すべてのアカウントは `{accountSlug}.gemihub.net` が即座に利用可能。カスタムドメインはオプションで追加可能。DNS 未設定のアカウントでもビルトインサブドメインで動作する。
...
hubwork-accounts/{accountId}
  ...
  accountSlug: string                — ユニーク、例: "acme" → acme.gemihub.net
  defaultDomain: string              — "acme.gemihub.net" (スラッグから自動生成)
...
初期の公開 URL は常に `https://{accountSlug}.gemihub.net` です。カスタムドメイン設定後は、両方の URL で同一のコンテンツが配信されます。
...
予約のキャンセル、無断キャンセル記録、問い合わせへの返信といった**運営者向け操作画面**は、公開サイト（`web/`）とは完全に分離して `admin/` 配下に置きます。カスタムドメインで一般公開されるのは `web/` のみで、`admin/` 配下はカスタムドメインからは自動的に 404 になります。Drive 所有者が GemiHub IDE のプレビューモードからのみ操作するため、他の入口は存在しません。
...
| 層 | URL | 利用可能条件 |
|----|-----|-------------|
| ビルトイン | `https://{accountSlug}.gemihub.net` | アカウント作成時に即座に利用可能 |
| カスタム | `https://app.acme.com` | DNS 検証 + SSL プロビジョニング完了後 |

管理パネルでは両方が表示されます:
- **プレビュー URL**: `https://{accountSlug}.gemihub.net` (常に動作)
