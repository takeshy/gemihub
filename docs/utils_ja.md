...
| メニュー | 説明 |
|---------|------|
| フォルダをキャッシュ (N) | フォルダ配下の uncached ファイルを一括でダウンロード・キャッシュする（ローカル専用の `new:` プレースホルダは除外）。`(N)` は取得予定件数。対象がある場合のみ表示。詳細は [Sync → Lazy-Fetch](./sync_ja.md#lazy-fetchuncached-ファイル) |
