...
- **サンドボックス実行**: `new Function` と制限された `require` シム（react, react-dom のみ）で実行
...
- CLI / シェルアクセスは不可
- 外部 API への直接ネットワークリクエストは不可（AI 呼び出しは `api.gemini` を使用）
- `require()` は `react`、`react-dom`、`react-dom/client` のみ提供
