...
| オプション | 説明 |
|-----------|------|
| **日時を追加** | 現在の日時 (`YYYY-MM-DD HH:MM:SS`) を YAML frontmatter の `date` フィールドとして挿入 |
| **位置情報を追加** | ブラウザの位置情報を取得し、`location` ブロック（`latitude` / `longitude`）として YAML frontmatter に挿入 |

- いずれのオプションも `.md` ファイルのみに適用される。それ以外のファイルタイプでは無視され、メタデータは挿入されない
...
2. 共有 URL を生成するかどうかの確認ダイアログが表示される
3. `/api/drive/temp` に現在のファイル内容をアップロード（Drive `__TEMP__` フォルダに保存）
