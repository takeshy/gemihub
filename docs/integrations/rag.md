...
RAG タブには OKF（Open Knowledge Format）ナレッジソースの設定も含まれる。OKF は常に有効で、`okfRoot`（Drive ルートからの相対フォルダパス、デフォルト `Knowledge`）を変更した場合のみ Save ボタンが有効になる。タブを開いたときと保存時に、その配下で発見されたバンドルの一覧が表示される。どのバンドルを使うかはチャット入力欄の上の OKF セレクタでチャットごとに選択し、選択バンドルの要約がチャットのシステムプロンプトに注入される（File Search への登録ではなくプロンプトコンテキストとしての注入）。詳細は [OKF.md](../references/OKF.md) を参照。
