...
description: Documentation for the RAG (Retrieval-Augmented Generation) feature.
tags:
  - rag
---
...
RAG タブには OKF（Open Knowledge Format）ナレッジソースの設定も含まれる。`okfRoot`（Drive ルートからの相対フォルダパス）を設定してバンドルを選択すると、選択バンドルの要約がチャットのシステムプロンプトに注入される（File Search への登録ではなくプロンプトコンテキストとしての注入）。詳細は [OKF.md](./OKF.md) を参照。
