...
description: RAG (Retrieval-Augmented Generation) setup and behavior using Gemini File Search.
tags:
  - rag
---
...
**RAG モードの詳細**:
- API プランに応じたモデル選択: paid → `gemini-3.8-flash` / `gemini-3.1-pro-preview`, free → `gemini-3.8-flash` / `gemini-3.5-flash-lite`
...
- API プランに応じたモデル選択ボタンが表示される
  - paid: `gemini-3.8-flash`, `gemini-3.1-pro-preview`
  - free: `gemini-3.8-flash`, `gemini-3.5-flash-lite`
...
RAG タブには OKF（Open Knowledge Format）ナレッジソースの設定も含まれる。OKF は常に有効で、`okfRoot`（Drive ルートからの相対フォルダパス、デフォルト `Knowledge`）を変更した場合のみ Save ボタンが有効になる。タブを開いたときと保存時に、その配下で発見されたバンドルの一覧が表示される。どのバンドルを使うかはチャット入力欄の上の OKF セレクタでチャットごとに選択し、選択バンドルの`index.md`がチャットのシステムプロンプトに注入される（File Search への登録ではなくプロンプトコンテキストとしての注入）。個々の文書の全文は、AIが`read_okf_document` toolで必要な時だけ取得する。詳細は [OKF.md](../references/OKF.md) を参照。
