...
- **Pull バッジ**: 既にキャッシュ済みファイルの更新（`toPull`）、コンフリクト、リモートで削除されたファイル（`localOnly`）など、手動対応が必要なリモート変更の数。新規リモートファイルはここにはカウントされず、バックグラウンド・ポーリング時に uncached エントリとして自動登録され、そのままファイルツリーに現れます。
...
アプリがアクティブでアイドル状態の間、クライアントは 5 分ごとにリモートの変更をポーリングします。新しい `_sync-meta.json` を取得したときの動き:

1. **キャッシュ済みリモートメタを更新**（ローカルの `new:` エントリは保持）。
2. **新規リモートファイルをローカル sync meta に自動登録** — metadata のみ（uncached）で登録され、次回 `tree-meta-updated` イベントでツリーに反映される。content はユーザーがファイルを開いた瞬間に遅延取得される。この挙動により、別デバイスがファイルを追加しただけで Pull バッジが増えなくなる。
3. **永続トーストを表示** — 過去に同期した履歴があり、新規エントリが登録された場合に、追加されたファイルパスを全件リスト（ソート済み）で表示する。ユーザーが手動で閉じるまで残るため、ツリーの深い階層に追加された場合でも見落とさない。初回同期時は全ファイルが「新規」扱いになるため発火しない。
4. **`tree-meta-updated` を発火** — リモートメタの `lastUpdatedAt` が変化したか、新規エントリが登録された場合に発火。手動 Pull を行わなくてもリモート側の追加・リネーム・削除がツリーに反映される。
5. **Pull / Push のカウントを再計算。**
...
- `remoteChanged = localMeta.md5Checksum !== remoteMeta.md5Checksum || localMeta.name !== remoteMeta.name` — 別のデバイスが Push した場合にリモートメタがローカルメタと異なる（コンテンツと名前の両方の変更を検出）
...
1. 事前チェック: 書き込み前に Diff を確認
   ...
   └─ リモートに Push をブロックする変更あり（コンフリクト、edit-delete コンフリクト、toPull、ローカルにキャッシュが残っているリモート削除エントリ）→ エラー「先に Pull して」。
       判定には Pull バッジと同じフィルタ（`filterActionablePull`）を適用: 同期除外パス、キャッシュ内容が既にリモートのチェックサムと一致しているファイル、キャッシュのない stale な削除エントリはブロックしない。純粋な新規リモートファイル（`remoteOnly`）も Push をブロックしない。

2. バッチアップロード: 全ファイルを単一 API コールで更新
   ├─ IndexedDB の editHistory から変更ファイル ID を取得（追跡済み・新規/未追跡を問わない。`new:` プレースホルダは除外）
   ├─ システムファイルや除外パス (history/, plugins/ など) を除外
   ├─ バイナリファイル (base64 エンコード) は encoding フラグ付きで含める (hasNetContentChange はスキップ)
   ├─ 差し戻しテキストファイルを除外 (hasNetContentChange = false)
   ...
   └─ 返された remoteMeta から、Push したファイルのエントリのみを LocalSyncMeta にマージ
       （他デバイスの並行変更は pending のまま残り、次回 Pull で反映される）
...
リモートで変更されたファイルをローカルキャッシュにダウンロードします。新規リモートファイルは metadata のみ（uncached）で登録され、開いた瞬間に遅延取得されます。

### フロー

1. **ローカルメタとリモートメタで Diff を計算**（`locallyModifiedFileIds` 付き）
2. **コンフリクト収集**（edit-delete コンフリクト含む）— 以下の非コンフリクト処理の完了後にコンフリクト UI に表示される
3. **`localOnly` ファイルのクリーンアップ** — ローカル同期メタで追跡されていてリモートで削除されたファイル（別デバイスでゴミ箱に移動されたもの）を IndexedDB キャッシュ、編集履歴、ローカル同期メタから削除。editHistory にしか存在しないエントリ（Push 待ちの新規ローカルファイル）と `new:` プレースホルダは対象外。
4. **`toPull` + `remoteOnly` 配列を結合**
5. **内容のダウンロードが不要なエントリはスキップ**: `remoteOnly`（新規リモートファイル）、モバイル環境でのバイナリファイル、100 MB 超のファイル、同期除外パス、キャッシュ内容が既にリモートのチェックサムと一致しているファイル（localMeta が stale なまま遅延取得済みのケース）。metadata はローカル同期メタに書き込まれるのでツリーには表示され、pending 扱いも解消される。
6. **残りのファイルを並列ダウンロード**（最大5並列）
...
| ローカルメタ | リモートメタ | アクション |
|:----------:|:----------:|--------|
| - | A | **remoteOnly** → metadata のみ登録（uncached）。content は最初に開いたときに遅延取得 |

---

## Lazy-Fetch（uncached ファイル）

ファイル metadata はローカル同期メタに登録されているが、IndexedDB の `files` ストアにエントリが無い状態を **uncached** と呼びます。uncached エントリが発生する経路は 3 つ:

| 発生源 | 理由 |
|--------|------|
| バックグラウンド・ポーリングでの自動登録 | 同期ポーリング間に検出した新規リモートファイル |
| 差分 Pull | Diff 結果の `remoteOnly` エントリ |
| モバイル / 大容量ファイルのスキップ | モバイルでのバイナリファイル、100 MB 超のファイル |

ユーザーが uncached ファイルを開くと、`useFileWithCache` が Drive から content を取得してキャッシュに書き込み、ファイルツリーが購読している `file-cached` イベントを発火します。追加の同期操作は不要です。

### 一括キャッシュ

最初に開くのを待たずに、uncached ファイルをまとめて取得できます:

- **フォルダ右クリック → "Cache folder (N)"**: 選択フォルダ配下の uncached ファイル（`new:` プレースホルダを除く）をまとめてキャッシュ。対象ファイルがある場合のみ表示。
- **FILES ヘッダー → Cache アイコン**: ワークスペース全体の uncached ファイルをキャッシュ。全ファイルがキャッシュ済みか一括キャッシュ実行中は disabled。ツールチップに未キャッシュ件数または進捗が出る。

どちらも `useSync.cacheFilesByIds` を経由し、ID を 200 件ずつチャンクに分割して `pullDirect` API を順次呼び、レスポンスをキャッシュに書き込みます。進捗は `cachingProgress`（フック公開）で観測でき、sync lock により他の同期操作と排他制御されます。
...
選ばれなかった方は必ずバックアップされるため、手動マージが可能です。バイナリファイルは base64 でラウンドトリップされます: 勝った側の内容はバイナリ update/create でアップロードされ、バイナリのバックアップも（base64 テキストではなく）実バイナリファイルとして保存されます。
...
「一時UP」でファイルを保存した後、編集URLを発行するかどうかの確認ダイアログが表示されます。「はい」を選ぶとURLがクリップボードにコピーされます。編集URLを使うと他のアプリからファイルを編集できます（有効期限1時間）。
...
| `app/routes/api.sync.tsx` | サーバー側の同期 API（18 POST アクション + GET ローダー） |
...
| `app/services/sync-diff.ts` | `computeSyncDiff` 実装（`sync-meta.server.ts` から再エクスポート） |
| `app/services/sync-client-utils.ts` | `isSyncExcludedPath`、`isBinaryMimeType`、バイナリ一時ファイルアップロード |
...
| `app/components/ide/SyncStatusBar.tsx` | Push/Pull バッジ、Diff ダイアログトリガー |
| `app/components/ide/SyncDiffDialog.tsx` | Push/Pull ファイルリスト（Diff プレビュー付き） |
| `app/components/ide/ConflictDialog.tsx` | コンフリクト解決 UI |

### API アクション

| アクション | メソッド | 説明 |
|-----------|---------|------|
| *（ローダー）* | GET | `remoteMeta`、`syncMetaFileId`、ファイルリストを返す |
| `pullDirect` | POST | 指定 ID のファイル内容をダウンロード（メタの読み書きなし） |
| `resolve` | POST | コンフリクト解決（敗者をバックアップ、Drive ファイルとメタを更新） |
| `fullPull` | POST | リモートの全ファイルをダウンロード（一致分はスキップ） |
| `pushFiles` | POST | 複数ファイルを Drive に並列バッチ更新。クライアントから `remoteMeta` と `syncMetaFileId` を受け取り、冗長なメタ読込/検索をスキップ |
...
| `rebuildTree` | POST | ファイルツリーを Drive から再構築（フルスキャン） |
| `migrateRootFolder` | POST | Drive 上のルートフォルダ名を移行 |
