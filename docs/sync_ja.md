...
1. 事前チェック: 書き込み前に Diff を確認
   ...
   └─ リモートに Push をブロックする変更あり（コンフリクト、edit-delete コンフリクト、toPull、ローカルメタにだけ残っているリモート削除エントリ）→ エラー「先に Pull して」。純粋な新規リモートファイル（`remoteOnly`）は Push をブロックしない。
...
5. **uncached 扱いのエントリはダウンロードをスキップ**: `remoteOnly`（新規リモートファイル）、モバイル環境でのバイナリファイル、100 MB 超のファイル。metadata はローカル同期メタに書き込まれるのでツリーには表示される。
