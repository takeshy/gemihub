...
各エントリの表示内容: タイムスタンプ、元バッジ（local/remote）、追加/削除の統計、展開可能な diff ビュー。最新でないエントリには **Copy (Save As)** ボタンがあり、その時点のコンテンツを再構築して新しい Drive ファイルとして保存できる。
...
呼び出し元（`EditHistoryModal.handleRestore`）がステップ 1-2 を実行します。`restoreToHistoryEntry`（ステップ 3-4）が復元内容を計算し編集履歴を更新します。呼び出し元がステップ 5-6 を実行します。

1. IndexedDB キャッシュから現在の内容を読み取り *（呼び出し元）*
2. 対象エントリより新しい diff を計算し `restoreToHistoryEntry` に渡す *（呼び出し元）*
3. 渡された非空 diff を逆適用（対象エントリ自体は逆適用しない）:
   - **ローカル diff**: 常に逆適用（`+`/`-` 行を入れ替え、ハンクヘッダーを反転し、パッチを適用）
   - **リモート diff**: 逆適用を試行し、失敗した場合（コンテンツが OLD 側＝未 Pull 状態）はスキップ
   - リストアを新しい履歴エントリとして記録: `diff(現在 → 復元後)`、コミット境界を追加
4. 復元内容を返す
