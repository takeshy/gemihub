...
function isErrorValue(v: unknown): v is ErrorValue {
  return typeof v === "object" && v !== null && (v as ErrorValue).type === "error";
}
...
function evalNode(node: AstNode, ctx: EvalContext): Value {
  switch (node.kind) {
    ...
    default:
      return errorVal("EVAL002", `Unknown AST node kind: ${(node as { kind: string }).kind}`);
  }
}
...
function resolveIdentifier(name: string, ctx: EvalContext): Value {
  ...
  if (name === "note" || name === "file" || name === "formula" || name === "this") {
...
function evalUnary(node: AstNode & { kind: "unary" }, ctx: EvalContext): Value {
  ...
  switch (node.op) {
    ...
    case "+":
      if (operand.type !== "number") return errorVal("TYPE001", "Unary + requires a number");
      return operand;
    case "-":
      if (operand.type !== "number") return errorVal("TYPE001", "Unary - requires a number");
...
function evalBinary(node: AstNode & { kind: "binary" }, ctx: EvalContext): Value {
  const op = node.op;
  ...
  switch (op) {
    ...
    case ">": return evalRelational(left, right, ">");
    case "<": return evalRelational(left, right, "<");
    case ">=": return evalRelational(left, right, ">=");
    case "<=": return evalRelational(left, right, "<=");
    case "+": return evalAdd(left, right, ctx);
    case "-": return evalSub(left, right, ctx);
    case "*": return evalMul(left, right, ctx);
    case "/": return evalDiv(left, right, ctx);
    case "%": return evalMod(left, right);
    default:
...
function evalRelational(left: Value, right: Value, op: string): Value {
  ...
  if (left.type === "date" && right.type === "date") {
    switch (op) {
      case ">": return bool(left.epochMs > right.epochMs);
      case "<": return bool(left.epochMs < right.epochMs);
      case ">=": return bool(left.epochMs >= right.epochMs);
      case "<=": return bool(left.epochMs <= right.epochMs);
    }
  ...
  if (left.type === "date" && right.type === "string") {
    ...
    if (parsed.type === "error") return errorVal("TYPE002", "Invalid relational comparison: cannot parse string as date");
  ...
  return errorVal("TYPE002", `Invalid relational comparison: ${left.type} ${op} ${right.type}`);
}
...
function evalAdd(left: Value, right: Value, ctx: EvalContext): Value {
  ...
  if (left.type === "string" || right.type === "string" ||
    ...
    if (!((left.type === "date" || left.type === "duration") && right.type === "string") &&
        !((right.type === "date" || right.type === "duration") && left.type === "string")) {
...
function evalMul(left: Value, right: Value, _ctx: EvalContext): Value {
  ...
  if (left.type === "number" && right.type === "duration") {
    return errorVal("TYPE003", "Number * Duration not supported; Duration must be on the left");
  }
  return errorVal("TYPE003", `Invalid arithmetic: ${left.type} * ${right.type}`);
}
...
function evalMember(node: AstNode & { kind: "member" }, ctx: EvalContext): Value {
...
function resolveFileFieldFromValue(file: FileValue, field: string, ctx: EvalContext): Value {
  switch (field) {
    case "name": return str(file.name);
    case "basename": return str(file.basename);
    case "path": return str(file.path);
...
function resolveDateField(date: DateValue, field: string, ctx: EvalContext): Value {
  const tz = ctx.snapshot.timezone;
  const offset = getTimezoneOffsetMs(tz, date.epochMs);
  const d = new Date(date.epochMs + offset);
  switch (field) {
    case "year": return num(d.getUTCFullYear());
    case "month": return num(d.getUTCMonth() + 1);
    case "day": return num(d.getUTCDate());
    case "hour": return num(d.getUTCHours());
    case "minute": return num(d.getUTCMinutes());
    case "second": return num(d.getUTCSeconds());
    case "millisecond": return num(d.getUTCMilliseconds());
    default: return NULL;
  }
}
...
function evalIndex(node: AstNode & { kind: "index" }, ctx: EvalContext): Value {
  ...
  if (node.object.kind === "identifier") {
    ...
    if (nsName === "formula" && ctx.rowScope) {
      ...
      return errorVal("REF003", "Dynamic formula key not statically resolvable");
    }
  }
...
function evalCall(node: AstNode & { kind: "call" }, ctx: EvalContext): Value {
  ...
  if (node.callee.kind === "identifier" && (node.callee as { name: string }).name === "if") {
  ...
  if (node.callee.kind === "index") {
    ...
    if (methodNameVal.type !== "string") return errorVal("TYPE001", "Method name must be a string");
...
function evalMethodCall(receiver: Value, methodName: string, args: AstNode[], ctx: EvalContext): Value {
  ...
  ctx.diagnostics.push({
    ...
    span: { startOffset: 0, endOffset: 0, startLine: 0, startColumn: 0, endLine: 0, endColumn: 0 },
  });
  return errorVal("NAME002", `Unknown method '${methodName}' on type '${receiver.type}'`);
}
...
function evalListFilter(list: ListValue, args: AstNode[], ctx: EvalContext): Value {
  if (args.length !== 1) return errorVal("ARITY001", "filter() expects 1 argument");
...
function evalListMap(list: ListValue, args: AstNode[], ctx: EvalContext): Value {
  if (args.length !== 1) return errorVal("ARITY001", "map() expects 1 argument");
  const mapAst = args[0];
  const result: Value[] = [];
  for (let i = 0; i < list.items.length; i++) {
    const itemCtx: EvalContext = {
...
function evalListReduce(list: ListValue, args: AstNode[], ctx: EvalContext): Value {
  if (args.length !== 2) return errorVal("ARITY001", "reduce() expects 2 arguments");
