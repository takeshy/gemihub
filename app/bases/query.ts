...
export function queryView(
  ...
  if (!view) {
    ...
    return { data: [], groupedData: [], properties: [], diagnostics, getSummaryValue: () => NULL };
  }

  // Compile filters
  const globalFilterAst = compileFilter(base.config.filters, "filters", diagnostics);
  const viewFilterAst = compileFilter(view.filters, `views[${viewName}]`, diagnostics);

  // Compile view sort (already validated as property IDs)
  const sortKeys = view.sort ?? [];

  // Enumerate files and create row contexts
  const files = host.enumerateFiles().sort((a, b) => a.path < b.path ? -1 : a.path > b.path ? 1 : 0);
  let entries: BaseEntry[] = files.map((hf) => createEntry(hf, host, snapshot, base));

  // Apply filters
  entries = entries.filter((entry) => {
    const ctx = makeEvalContext(snapshot, host, entry, diagnostics);
    return evalFilter(globalFilterAst, entry, ctx, diagnostics) && evalFilter(viewFilterAst, entry, ctx, diagnostics);
  });
  ...
  if (view.groupBy) {
    groupedData = groupEntries(entries, view.groupBy.property, view.groupBy.direction, host, snapshot, diagnostics);
  ...
  const getSummaryValue = (entries: BaseEntry[], property: string, summary: string): Value => {
    const values = entries.map((e) => resolveProperty(e, property, host, snapshot, diagnostics));
    return computeSummary(summary, values, base, host, snapshot, diagnostics);
  };

  return { data: entries, groupedData, properties, diagnostics, getSummaryValue };
}
...
type CompiledFilter = { ast: AstNode | null; children: CompiledFilter[] | null; type: "expr" | "and" | "or" | "not" };

function compileFilter(node: FilterNode | undefined, section: string, diagnostics: Diagnostic[]): CompiledFilter {
  ...
  if (typeof node === "string") {
    try {
      ...
      const code = e instanceof LexError ? e.code : e instanceof ParseError ? e.code : "EVAL002";
      diagnostics.push({
        code,
        severity: "error",
  ...
  if (typeof node === "object" && node !== null) {
    const obj = node as { and?: FilterNode[]; or?: FilterNode[]; not?: FilterNode[] };
    if (obj.and) {
      return { ast: null, children: obj.and.map((c) => compileFilter(c, section, diagnostics)), type: "and" };
    }
    if (obj.or) {
      return { ast: null, children: obj.or.map((c) => compileFilter(c, section, diagnostics)), type: "or" };
    }
    if (obj.not) {
      return { ast: null, children: obj.not.map((c) => compileFilter(c, section, diagnostics)), type: "not" };
    }
  }
  return { ast: null, children: null, type: "expr" };
}
...
function evalFilter(
  ...
  if (filter.type === "expr") {
    ...
    try {
      ...
      diagnostics.push({ code: err.code, severity: "error", message: err.message });
      return false;
    }
  }
...
function stableSort(
  ...
  indexed.sort((a, b) => {
    for (const key of sortKeys) {
      const va = resolveProperty(a.entry, key.property, host, snapshot, diagnostics);
      const vb = resolveProperty(b.entry, key.property, host, snapshot, diagnostics);
...
function groupEntries(
  ...
  for (const entry of entries) {
    ...
    const keyStr = key.type === "string" || key.type === "number" || key.type === "boolean" || key.type === "null"
      ? valueToString(key)
      : JSON.stringify(key);
  ...
  return result.map((g) => ({ key: g.key, entries: g.entries, summaries: new Map() }));
}
...
function resolveFileField(field: string, entry: BaseEntry, host: BasesHostAdapter): Value {
  const file = entry.rowScope.file;
  switch (field) {
    case "name": return str(file.name);
    case "basename": return str(file.basename);
    case "path": return str(file.path);
    case "folder": return str(file.folder);
    case "ext": return str(file.ext);
    case "size": return num(file.size);
    case "ctime": return dateVal(file.ctimeMs, false);
    case "mtime": return dateVal(file.mtimeMs, false);
    case "file": return file;
    case "properties": return entry.rowScope.note;
    case "tags": return listVal(host.getTags(entry.rowScope.fileMeta).map((t) => str(t)));
    case "links": return listVal(host.getOutgoingLinks(entry.rowScope.fileMeta).map((l) => ({
      type: "link" as const, target: l.target, sourcePath: file.path, resolvedPath: l.resolvedPath,
    })));
    case "backlinks": return listVal(host.getBacklinks(entry.rowScope.fileMeta).map((bf) => ({
      type: "file" as const, path: bf.path, name: bf.name, basename: bf.basename, ext: bf.extension,
      folder: bf.path.includes("/") ? bf.path.substring(0, bf.path.lastIndexOf("/")) : "",
      size: bf.size, ctimeMs: bf.ctimeMs, mtimeMs: bf.mtimeMs,
    })));
    case "embeds": return listVal(host.getEmbeds(entry.rowScope.fileMeta).map((l) => ({
      type: "link" as const, target: l.target, sourcePath: file.path, resolvedPath: l.resolvedPath,
    })));
    default: return NULL;
  }
...
function computeSummary(
  ...
  // Built-in summaries
  switch (summaryName) {
    case "Average": return summaryAverage(values);
    case "Min": return summaryMin(values);
    case "Max": return summaryMax(values);
    case "Sum": return summarySum(values);
    case "Range": return summaryRange(values);
    case "Median": return summaryMedian(values);
    case "Stddev": return summaryStddev(values);
    case "Earliest": return summaryEarliest(values);
    case "Latest": return summaryLatest(values);
    case "Checked": return summaryChecked(values);
    case "Unchecked": return summaryUnchecked(values);
    case "Empty": return summaryEmpty(values);
    case "Filled": return summaryFilled(values);
    case "Unique": return summaryUnique(values);
    default: return errorVal("SUMMARY001", `Unknown summary: ${summaryName}`);
  }
}
...
function summaryRange(values: Value[]): Value {
  ...
  if (nums.length > 0 && dates.length > 0) {
    return errorVal("SUMMARY002", "Range has ambiguous input type: number and date");
  }
  if (nums.length > 0) {
...
function collectProperties(base: CompiledBase, view: ViewConfig, entries: BaseEntry[]): string[] {
