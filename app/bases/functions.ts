...
export const globalFunctions: Record<string, GlobalFn> = {
  escapeHTML(args: Value[]): Value {
    ...
    if (s.type !== "string") return errorVal("TYPE001", "escapeHTML expects a string");
  ...
  date(args: Value[], ctx: EvalContext): Value {
    ...
    if (input.type === "string") return parseDate(input.value, ctx.snapshot.timezone);
  ...
  file(args: Value[], ctx: EvalContext): Value {
    ...
    if (input.type === "link") {
      ...
      const resolved = ctx.host.resolveLink(stripWikilink(input.target), input.sourcePath);
  ...
  html(args: Value[], ctx: EvalContext): Value {
    const input = args[0];
    if (input.type === "error") return input;
    if (input.type !== "string") return errorVal("TYPE001", "html() expects a string");
  ...
  image(args: Value[], ctx: EvalContext): Value {
    ...
    if (input.type === "file") return { type: "image", source: input.path, resolvedPath: input.path };
    if (input.type === "link") return { type: "image", source: input.target, resolvedPath: input.resolvedPath };
  ...
  icon(args: Value[], ctx: EvalContext): Value {
    const input = args[0];
    if (input.type === "error") return input;
    if (input.type !== "string") return errorVal("TYPE001", "icon() expects a string");
    if (!ctx.host.isSupportedIcon(input.value)) return errorVal("RES002", `Unknown icon: ${input.value}`);
  ...
  link(args: Value[], ctx: EvalContext): Value {
    ...
    if (input.type === "link") {
      return { ...input, display: display && display.type !== "null" ? display : undefined };
    }
    if (input.type === "url") {
      return { ...input, display: display && display.type !== "null" ? display : undefined };
    }
    if (input.type === "string") {
      ...
      if (/^(https?|mailto):/i.test(s)) {
        const url: UrlValue = { type: "url", url: canonicalizeUrl(s), display: display && display.type !== "null" ? display : undefined };
        return url;
      }
      // Internal link
      ...
      const link: LinkValue = {
        ...
        display: display && display.type !== "null" ? display : (wl?.display ? str(wl.display) : undefined),
      };
      return link;
    }
  ...
  max(args: Value[]): Value {
    ...
    for (const a of args) if (a.type !== "number") return errorVal("TYPE001", "max() expects numbers");
    if (args.length === 0) return num(-Infinity);
    for (const a of args) if (a.type === "number" && Number.isNaN(a.value)) return num(NaN);
    return num(Math.max(...args.map((a) => (a as NumberValue).value)));
  },

  min(args: Value[]): Value {
    for (const a of args) if (a.type === "error") return a;
    for (const a of args) if (a.type !== "number") return errorVal("TYPE001", "min() expects numbers");
    if (args.length === 0) return num(Infinity);
    for (const a of args) if (a.type === "number" && Number.isNaN(a.value)) return num(NaN);
  ...
  number(args: Value[]): Value {
    ...
    if (input.type === "string") {
      ...
      if (Number.isNaN(n)) return errorVal("TYPE004", `Cannot convert string to number: ${input.value}`);
      return num(n);
    }
...
export const commonMethods: Record<string, MethodFn> = {
  ...
  isType(receiver: Value, args: Value[]): Value {
    ...
    if (t.type !== "string") return errorVal("TYPE001", "isType expects a string argument");
...
export const typeMethods: Record<string, Record<string, MethodFn>> = {
  ...
  string: {
    contains(receiver: StringValue, args: Value[]): Value {
      ...
      if (sub.type !== "string") return errorVal("TYPE001", "contains expects a string");
      return bool(receiver.value.includes(sub.value));
    },
    containsAll(receiver: StringValue, args: Value[]): Value {
      for (const a of args) {
        if (a.type === "error") return a;
        if (a.type !== "string") return errorVal("TYPE001", "containsAll expects strings");
    ...
    containsAny(receiver: StringValue, args: Value[]): Value {
      for (const a of args) {
        if (a.type === "error") return a;
        if (a.type !== "string") return errorVal("TYPE001", "containsAny expects strings");
    ...
    endsWith(receiver: StringValue, args: Value[]): Value {
      ...
      if (q.type !== "string") return errorVal("TYPE001", "endsWith expects a string");
    ...
    replace(receiver: StringValue, args: Value[]): Value {
      ...
      if (replacement.type !== "string") return errorVal("TYPE001", "replace replacement must be a string");

      if (pattern.type === "string") {
        // Replace all occurrences
        const escaped = pattern.value.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
        return str(receiver.value.replace(new RegExp(escaped, "g"), replacement.value));
      }
    ...
    repeat(receiver: StringValue, args: Value[]): Value {
      ...
      if (count.type !== "number") return errorVal("TYPE001", "repeat expects a number");
      if (!Number.isInteger(count.value) || count.value < 0) return errorVal("TYPE005", "repeat count must be a non-negative integer");
      try {
    ...
    slice(receiver: StringValue, args: Value[]): Value {
      ...
      if (start.type !== "number") return errorVal("TYPE001", "slice start must be a number");
      if (end && end.type === "error") return end;
      if (end && end.type !== "number" && end.type !== "null") return errorVal("TYPE001", "slice end must be a number");
      const s = receiver.value;
      const startIdx = start.value < 0 ? Math.max(0, s.length + start.value) : Math.min(start.value, s.length);
      const endIdx = !end || end.type === "null"
        ? s.length
        : end.value < 0 ? Math.max(0, s.length + end.value) : Math.min(end.value, s.length);
    ...
    split(receiver: StringValue, args: Value[]): Value {
      ...
      if (sep.type === "string") {
        ...
        return errorVal("TYPE001", "split separator must be a string or regexp");
      }

      if (n) {
        if (n.type === "error") return n;
        if (n.type !== "number") return errorVal("TYPE001", "split limit must be a number");
    ...
    startsWith(receiver: StringValue, args: Value[]): Value {
      const q = args[0];
      if (q.type === "error") return q;
      if (q.type !== "string") return errorVal("TYPE001", "startsWith expects a string");
  ...
  number: {
    ...
    round(receiver: NumberValue, args: Value[]): Value {
      const digits = args[0];
      const d = !digits || digits.type === "null" ? 0 : (digits as NumberValue).value;
      if (!Number.isInteger(d) || d < 0 || d > 100) return errorVal("TYPE005", "round digits must be 0..100");
    ...
    toFixed(receiver: NumberValue, args: Value[]): Value {
      ...
      const p = !precision || precision.type === "null" ? 0 : (precision as NumberValue).value;
      if (!Number.isInteger(p) || p < 0 || p > 100) return errorVal("TYPE005", "toFixed precision must be 0..100");
  ...
  date: {
    ...
    format(receiver: DateValue, args: Value[], ctx: EvalContext): Value {
      ...
      if (fmt.type !== "string") return errorVal("TYPE001", "format expects a string");
    ...
    relative(receiver: DateValue, args: Value[], ctx: EvalContext): Value {
      ...
      if (absDiff < 60000) return str(`${Math.round(absDiff / 1000)} seconds${sign || ago}`);
      if (absDiff < 3600000) return str(`${Math.round(absDiff / 60000)} minutes${sign || ago}`);
      if (absDiff < 86400000) return str(`${Math.round(absDiff / 3600000)} hours${sign || ago}`);
  ...
  list: {
    ...
    join(receiver: ListValue, args: Value[]): Value {
      const sep = args[0];
      if (sep.type === "error") return sep;
      if (sep.type !== "string") return errorVal("TYPE001", "join expects a string separator");
      return str(receiver.items.map((item) => (item.type === "null" ? "" : valueToString(item))).join(sep.value));
    },
    ...
    slice(receiver: ListValue, args: Value[]): Value {
      ...
      const startIdx = start.value < 0 ? Math.max(0, len + start.value) : Math.min(start.value, len);
      const endIdx = !end || end.type === "null"
        ? len
        : (end as NumberValue).value < 0 ? Math.max(0, len + (end as NumberValue).value) : Math.min((end as NumberValue).value, len);
    ...
    mean(receiver: ListValue): Value {
      const numbers = receiver.items.filter((v) => v.type === "number") as NumberValue[];
  ...
  link: {
    asFile(receiver: LinkValue, _args: Value[], ctx: EvalContext): Value {
      ...
      const resolved = ctx.host.resolveLink(stripWikilink(receiver.target), receiver.sourcePath);
      if (resolved) return hostFileToFileValue(resolved);
      return NULL;
    },
    linksTo(receiver: LinkValue, args: Value[], ctx: EvalContext): Value {
      ...
      if (!receiverFile) {
        receiverFile = ctx.host.resolveLink(stripWikilink(receiver.target), receiver.sourcePath);
      }
      ...
      if (target.type === "file") {
        ...
        if (target.resolvedPath) targetFile = ctx.host.getFile(target.resolvedPath);
        if (!targetFile) targetFile = ctx.host.resolveLink(stripWikilink(target.target), target.sourcePath);
      } else if (target.type === "string") {
        targetFile = ctx.host.resolveLink(stripWikilink(target.value), receiverFile.path);
      }
  ...
  file: {
    ...
    hasLink(receiver: FileValue, args: Value[], ctx: EvalContext): Value {
      ...
      if (other.type === "file") {
        ...
        if (other.resolvedPath) otherFile = ctx.host.getFile(other.resolvedPath);
        if (!otherFile) otherFile = ctx.host.resolveLink(stripWikilink(other.target), other.sourcePath);
      } else if (other.type === "string") {
        otherFile = ctx.host.resolveLink(stripWikilink(other.value), sourceFile.path);
      }
    ...
    hasProperty(receiver: FileValue, args: Value[], ctx: EvalContext): Value {
      ...
      if (name.type !== "string") return errorVal("TYPE001", "hasProperty expects a string");
    ...
    hasTag(receiver: FileValue, args: Value[], ctx: EvalContext): Value {
      ...
      for (const arg of args) {
        ...
        if (arg.type !== "string") return errorVal("TYPE001", "hasTag expects strings");
    ...
    inFolder(receiver: FileValue, args: Value[]): Value {
      ...
      if (folder.type !== "string") return errorVal("TYPE001", "inFolder expects a string");
  ...
  regexp: {
    matches(receiver: RegexpValue, args: Value[]): Value {
      const input = args[0];
      if (input.type === "error") return input;
      if (input.type !== "string") return errorVal("TYPE001", "matches expects a string");
...
function hasLinkHelper(source: HostFile, target: HostFile, ctx: EvalContext): boolean {
...
export function hostFileToFileValue(hf: HostFile): FileValue {
  ...
  const folder = hf.path.includes("/") ? hf.path.substring(0, hf.path.lastIndexOf("/")) : "";
  return {
    type: "file",
    path: hf.path,
    name: hf.name,
...
function formatDate(date: DateValue, format: string, timezone: string): string {
  ...
  while (i < format.length) {
    ...
    switch (c) {
      ...
      case "M": result += len === 2 ? String(month).padStart(2, "0") : String(month); break;
      case "D": result += len === 2 ? String(day).padStart(2, "0") : String(day); break;
      case "H": result += len === 2 ? String(hours).padStart(2, "0") : String(hours); break;
      case "m": result += len === 2 ? String(minutes).padStart(2, "0") : String(minutes); break;
      case "s": result += len === 2 ? String(seconds).padStart(2, "0") : String(seconds); break;
      case "S": result += String(ms).padStart(3, "0").slice(0, len); break;
