...
export function compileCustomSummaries(
  ...
  for (const [name, expr] of Object.entries(summaries)) {
    try {
      const ast = parseExpression(expr);
      compiled.set(name, { name, expression: expr, ast, dependencies: new Set() });
    } catch (e) {
      const code = e instanceof LexError ? e.code : e instanceof ParseError ? e.code : "EVAL002";
      diagnostics.push({
        code,
        severity: "error",
        message: `Custom summary '${name}' parse error: ${(e as Error).message}`,
...
function collectRefs(node: AstNode, refs: Set<string>): void {
  switch (node.kind) {
    ...
    case "member":
      // formula.<name>
      if (node.object.kind === "identifier" && (node.object as { name: string }).name === "formula") {
...
export function detectCycles(compiled: Map<string, CompiledFormula>, diagnostics: Diagnostic[]): void {
...
export function createFormulaNamespace(
  ...
  function resolve(name: string): Value | undefined {
    ...
    if (ctx.evaluatingFormulas?.has(name)) {
      const err = errorVal("REF001", `Formula cycle detected at runtime: ${name}`);
...
export function compileBase(yamlText: string): CompiledBase {
  ...
  if (config) {
    ...
    for (const [name, expr] of Object.entries(config.summaries)) {
      try {
        const ast = parseExpression(expr);
        customSummaries.set(name, { name, expression: expr, ast, dependencies: new Set() });
      } catch (e) {
        const code = e instanceof LexError ? e.code : e instanceof ParseError ? e.code : "EVAL002";
        diagnostics.push({
          code,
  ...
  return {
    profile: "OBX-2026-06",
    config: config ?? { formulas: {}, properties: {}, summaries: {}, views: [] },
    formulas,
    customSummaries,
    diagnostics,
  };
