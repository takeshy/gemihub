...
export function isTruthy(v: Value): boolean {
  switch (v.type) {
    ...
    case "url": return v.url.length > 0;
    case "duration": return v.months !== 0 || v.milliseconds !== 0;
    ...
    case "error": throw v; // propagate error
  }
}
...
export function isEmpty(v: Value): boolean {
  switch (v.type) {
    ...
    case "object": return v.map.size === 0;
    case "error": throw v;
...
export function valueToString(v: Value): string {
  switch (v.type) {
    ...
    case "duration": return durationToString(v);
    ...
    case "link": return v.display ? valueToString(v.display) : v.target;
    case "url": return v.display ? valueToString(v.display) : v.url;
    case "regexp": return `/${v.source}/${v.flags}`;
    case "html": return v.source;
    case "image": return v.source;
    case "icon": return v.name;
    case "error": return v.message;
  }
}
...
function valueToJson(v: Value): string {
  switch (v.type) {
    ...
    case "list": return `[${v.items.map(valueToJson).join(",")}]`;
    case "object": {
      const entries: string[] = [];
      for (const [k, val] of v.map) {
        entries.push(`${JSON.stringify(k)}:${valueToJson(val)}`);
    ...
    default: return JSON.stringify(valueToString(v));
  }
}
...
function durationToString(d: DurationValue): string {
  ...
  if (d.months) {
    ...
    if (absM >= 12) {
      ...
      else parts.push(`${years} year${years > 1 ? "s" : ""} ${months} month${months > 1 ? "s" : ""}`);
    } else {
  ...
  if (d.milliseconds) {
    ...
    else if (hr % 24 > 0) parts.push(`${hr % 24} hour${hr % 24 > 1 ? "s" : ""}`);
    else if (min % 60 > 0) parts.push(`${min % 60} minute${min % 60 > 1 ? "s" : ""}`);
    else if (sec % 60 > 0) parts.push(`${sec % 60} second${sec % 60 > 1 ? "s" : ""}`);
...
export function looseEquals(a: Value, b: Value): boolean | ErrorValue {
  ...
  // Duration
  if (a.type === "duration" && b.type === "duration") return a.months === b.months && a.milliseconds === b.milliseconds;
...
export function compareValues(a: Value, b: Value): number {
  ...
  switch (a.type) {
    ...
    case "string":
      ...
      if (b.type === "url") return a.value < b.url ? -1 : a.value > b.url ? 1 : 0;
      return 0;
    case "url":
      if (b.type === "url") return a.url < b.url ? -1 : a.url > b.url ? 1 : 0;
      if (b.type === "string") return a.url < b.value ? -1 : a.url > b.value ? 1 : 0;
      return 0;
    case "file":
      if (b.type === "file") return a.path < b.path ? -1 : a.path > b.path ? 1 : 0;
      if (b.type === "link") {
    ...
    default:
      ...
      return valueToString(a) < valueToString(b) ? -1 : valueToString(a) > valueToString(b) ? 1 : 0;
  }
}
...
export function parseDuration(input: string): DurationValue | ErrorValue {
  ...
  while ((match = re.exec(rest)) !== null) {
    ...
    if (spec.months) {
      ...
      months += value * (unit === "y" || unit === "year" || unit === "years" ? 12 : 1);
    } else {
...
function parseIsoDuration(s: string): DurationValue | ErrorValue {
  ...
  let months = 0;
  let milliseconds = 0;
  if (m[1]) { months += parseFloat(m[1]) * 12; if (!Number.isInteger(parseFloat(m[1]))) return errorVal("DUR002", "Fractional year not supported"); }
  if (m[2]) { months += parseFloat(m[2]); if (!Number.isInteger(parseFloat(m[2]))) return errorVal("DUR002", "Fractional month not supported"); }
...
export function parseDate(input: string, timezone: string): DateValue | ErrorValue {
  const s = input.trim();
  if (s === "") return errorVal("DATE001", "Empty date string");

  // YYYYMMDDHHmm
  if (/^\d{12}$/.test(s)) {
    return makeDateValFromParts(Number(s.slice(0, 4)), Number(s.slice(4, 6)), Number(s.slice(6, 8)), Number(s.slice(8, 10)), Number(s.slice(10, 12)), 0, 0, timezone, false, s);
  }
  // YYYYMMDD
  if (/^\d{8}$/.test(s)) {
    return makeDateValFromParts(Number(s.slice(0, 4)), Number(s.slice(4, 6)), Number(s.slice(6, 8)), 0, 0, 0, 0, timezone, true, s);
  }
  ...
  return makeDateValFromParts(year, month, day, hour, minute, second, ms, timezone, !hasTime, s, tz);
}

function makeDateValFromParts(
  year: number, month: number, day: number,
  hour: number, minute: number, second: number, ms: number,
  timezone: string, dateOnly: boolean, original: string,
  explicitTz?: string,
): DateValue | ErrorValue {
  // Validate ranges
  if (month < 1 || month > 12) return errorVal("DATE001", `Invalid month: ${month}`);
  ...
  if (day > daysInMonth) return errorVal("DATE001", `Date does not exist: ${original}`);
  ...
  if (explicitTz === "Z") {
    ...
    epochMs = Date.UTC(year, month - 1, day, hour, minute, second, ms) - sign * (tzH * 3600000 + tzM * 60000);
  } else if (dateOnly) {
...
export function addDurationToDate(date: DateValue, duration: DurationValue, timezone: string): DateValue {
  ...
  const isMidnight = newLocal.getUTCHours() === 0 && newLocal.getUTCMinutes() === 0 && newLocal.getUTCSeconds() === 0 && newLocal.getUTCMilliseconds() === 0;
...
export function subtractDurationFromDate(date: DateValue, duration: DurationValue, timezone: string): DateValue {
  return addDurationToDate(date, { type: "duration", months: -duration.months, milliseconds: -duration.milliseconds }, timezone);
}

export function addDurations(a: DurationValue, b: DurationValue): DurationValue {
  return durVal(a.months + b.months, a.milliseconds + b.milliseconds);
}

export function subtractDurations(a: DurationValue, b: DurationValue): DurationValue {
  return durVal(a.months - b.months, a.milliseconds - b.milliseconds);
}
...
export function divideDuration(d: DurationValue, n: number): DurationValue | ErrorValue {
  if (n === 0) return errorVal("DUR003", "Division by zero in duration scalar arithmetic");
  let months = d.months / n;
  let milliseconds = d.milliseconds / n;
  if (!Number.isInteger(months)) {
    milliseconds += months * 30 * 86400000;
...
export function durationGetMilliseconds(d: DurationValue, referenceDate: DateValue, timezone: string): number {
...
export function detectWikilink(s: string): { target: string; display?: string } | null {
  const m = s.match(/^\[\[([^\]]+)\]\]$/);
  if (!m) return null;
  const pipeIdx = m[1].indexOf("|");
  if (pipeIdx >= 0) {
    return { target: m[1].substring(0, pipeIdx), display: m[1].substring(pipeIdx + 1) };
  }
  return { target: m[1] };
}
...
export function rawToValue(
  ...
  key: string,
  hostFile: HostFile,
  host: { resolveLink: (target: string, sourcePath: string) => HostFile | null },
): Value {
  ...
  if (Array.isArray(raw)) {
    return listVal(raw.map((item) => rawToValue(item, propertyTypes, key, hostFile, host)));
  }
...
export function valueToCanonical(v: Value): unknown {
  switch (v.type) {
    ...
    case "date": return { type: "date", epochMs: v.epochMs, dateOnly: v.dateOnly };
    case "duration": return { type: "duration", months: v.months, milliseconds: v.milliseconds };
    case "list": return { type: "list", value: v.items.map(valueToCanonical) };
    case "object": {
      const obj: Record<string, unknown> = {};
      for (const [k, val] of v.map) obj[k] = valueToCanonical(val);
      return { type: "object", value: obj };
    }
    case "file": return { type: "file", path: v.path };
    case "link": {
    ...
    case "regexp": return { type: "regexp", source: v.source, flags: v.flags };
    case "html": return { type: "html", source: v.source };
    case "image": {
      const r: Record<string, unknown> = { type: "image", source: v.source };
      if (v.resolvedPath) r.resolvedPath = v.resolvedPath;
      return r;
    }
    case "icon": return { type: "icon", name: v.name };
    case "error": return { type: "error", code: v.code };
  }
}
