    ...
    <>
      ...
      <figure className="mb-8 overflow-hidden rounded-xl border border-gray-200 shadow dark:border-gray-800">
        ...
        <figcaption className="px-3 py-2 text-xs text-gray-500 dark:text-gray-400">
          Sample: building a Hubwork site end-to-end through chat.
        </figcaption>
      </figure>
      <div className={prose}>
        <p>
          The <strong>Webpage Builder</strong> is a built-in Agent Skill that ships with the Pro plan and turns the chat into an end-to-end Hubwork site author. You describe what you want — a booking page, a member portal, a contact form — and the AI produces the HTML, the workflow YAML behind it, the sheet schema it needs, and the spec / change-log entries to go with it.
        </p>
        <p>
...
admin/                                            ← IDE-only operator pages
  ...
  api/<endpoint>.yaml                             ← admin workflows`}</code></pre>
        <p>
          The split is deliberate: <code>web/</code> is what the public Hubwork serving layer exposes on your custom domain; <code>admin/</code> is reachable only from the IDE preview, under the Drive owner&apos;s OAuth.
        </p>
  ...
  return (
    <>
      ...
      <figure className="mb-8 overflow-hidden rounded-xl border border-gray-200 shadow dark:border-gray-800">
        ...
        <figcaption className="px-3 py-2 text-xs text-gray-500 dark:text-gray-400">
          サンプル: チャットで Hubwork サイトをエンドツーエンドに構築する様子。
        </figcaption>
      </figure>
      <div className={prose}>
        <p>
          <strong>Webpage Builder</strong> は Pro プランに同梱される組み込み Agent Skill で、チャットを Hubwork サイトのエンドツーエンド作成ツールに変えます。「予約ページが欲しい」「会員制ポータルを作りたい」「問い合わせフォームを置きたい」と話すだけで、AI が HTML、裏側のワークフロー YAML、必要なシートスキーマ、仕様書 / 変更履歴まで一式生成します。
        </p>
        <p>
...
admin/                                            ← IDE 専用の運営者ページ
  ...
  api/<endpoint>.yaml                             ← admin ワークフロー`}</code></pre>
        <p>
          この分割は意図的です: <code>web/</code> はカスタムドメインで Hubwork のサーブ層が公開する領域、<code>admin/</code> は IDE プレビューからしか到達できず、Drive 所有者の OAuth で動きます。
        </p>
