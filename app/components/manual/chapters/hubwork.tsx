...
function HubworkEn() {
  return (
    <>
      <h1 className="mb-6 text-3xl font-bold text-gray-900 dark:text-gray-50">Hubwork (Paid Plans)</h1>
      <div className={prose}>
        <p>
          Hubwork is GemiHub&apos;s paid tier that transforms it into a web app builder with Google Sheets/Gmail integration, static page hosting, scheduled workflows, and custom domain support.
        </p>
        ...
        <ol>
          <li>Open <strong>Settings &gt; Hubwork</strong> and click <strong>Subscribe</strong>.</li>
          <li>Complete payment on the Stripe checkout page.</li>
          <li>Return to Settings — Hubwork features are automatically enabled.</li>
        </ol>
        <p>
          Manage your subscription (cancel, update payment method) from the Stripe Billing Portal link in the Hubwork settings tab.
        </p>
        ...
        <p>
          Configure the Spreadsheet ID in <strong>Settings &gt; Hubwork</strong>. Sheets nodes use the tab name and JSON filter syntax.
        </p>
        ...
        <ol>
          <li>Enter your domain in <strong>Settings &gt; Hubwork &gt; Custom Domain</strong>.</li>
...
<script>
  (async () => {
    ...
    document.getElementById("app").innerHTML = \`Welcome, \${user.email}\`;
        ...
        <p>
          Run workflows on a cron schedule. Configure in <strong>Settings &gt; Hubwork &gt; Schedules</strong>:
        </p>
        <ul>
        ...
        <p>
          A built-in Agent Skill (&quot;Hubwork Web&quot;) that generates web pages and API endpoints through AI chat. Activate it from the Skills dropdown in Chat, then describe what you want to build. The AI creates HTML pages and workflow YAML files, then saves them to your Drive.
        </p>
      </div>
    </>
  );
...
function HubworkJa() {
  return (
    <>
      <h1 className="mb-6 text-3xl font-bold text-gray-900 dark:text-gray-50">Hubwork（有料プラン）</h1>
      <div className={prose}>
        <p>
          Hubworkは、GemiHubの有料プランです。Google Sheets/Gmail連携、静的ページホスティング、スケジュール実行、カスタムドメインなどの機能により、Webアプリビルダーとして利用できます。
        </p>
        ...
        <h2>はじめ方</h2>
        <ol>
          <li><strong>設定 &gt; Hubwork</strong> を開き、<strong>Subscribe</strong>をクリック。</li>
          <li>Stripeの決済ページで支払いを完了。</li>
          <li>設定に戻ると、Hubwork機能が自動的に有効化されます。</li>
        </ol>
        <p>
          サブスクリプションの管理（解約、支払い方法の変更）は、Hubwork設定タブのStripe Billing Portalリンクから行えます。
        </p>
        ...
        <p>
          <strong>設定 &gt; Hubwork</strong> でスプレッドシートIDを設定します。Sheetsノードはタブ名とJSONフィルタ構文を使用します。
        </p>
        ...
        <ol>
          <li><strong>設定 &gt; Hubwork &gt; カスタムドメイン</strong>にドメインを入力。</li>
...
<script>
  (async () => {
    ...
    document.getElementById("app").innerHTML = \`Welcome, \${user.email}\`;
        ...
        <h3>スケジュール実行</h3>
        <p>
          cronスケジュールでワークフローを自動実行。<strong>設定 &gt; Hubwork &gt; スケジュール</strong>で設定：
        </p>
        <ul>
        ...
        <h3>AI Webビルダー</h3>
        <p>
          AIチャットでWebページやAPIエンドポイントを生成する組み込みAgent Skill（「Hubwork Web」）。チャットのSkillsドロップダウンから有効化し、作りたいものを説明するだけで、AIがHTMLページとワークフローYAMLファイルを作成してDriveに保存します。
        </p>
      </div>
    </>
  );
