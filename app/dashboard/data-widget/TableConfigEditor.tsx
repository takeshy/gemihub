// Config editor for the `table` widget (folder source → editable table).
...
export function TableConfigEditor({ config, onChange }: ConfigEditorProps) {
  const { t } = useI18n();
  const cfg = useMemo(() => (config ?? {}) as TableWidgetConfig, [config]);
  ...
  const update = useCallback(
    (patch: Partial<TableWidgetConfig>) => onChange({ ...cfg, ...patch }),
    [cfg, onChange],
  );
