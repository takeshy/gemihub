...
export function useGridLayout({
  ...
  // Layout positions for the current breakpoint.
  // For sm, missing positions are auto-derived via the shared deriveSmLayout
  // (same logic used at save time) so render and persistence stay in sync.
  const layout = useMemo(() => {
    ...
    return deriveSmLayout(data).widgets.map((w) => ({
      widget: w,
      pos: w.layout.sm ?? { x: 0, y: 0, w: 12, h: 3 },
    }));
  }, [data, breakpoint]);

  // Effective position map for the current breakpoint (used by compute functions)
  const posMapRef = useRef<Map<string, LayoutPos>>(new Map());
  posMapRef.current = new Map(layout.map((item) => [item.widget.id, item.pos]));

  /**
   * Compute new drag position from pixel delta.
   * Uses the effective position for the current breakpoint (including auto-derived sm).
   * Clamps to grid bounds (x >= 0, x + w <= cols, y >= 0).
   */
  ...
  /**
   * Compute new resize dimensions from pixel delta.
   * Clamps w to [1, cols - x], h to [1, infinity).
   */
  ...
  /**
   * Commit a new position for a widget.
   * Handles collision avoidance and calls onCommit with the updated data.
   */
