...
test("estimateVertexCostUsd prices input and output/thinking separately", () => {
  ...
  assert.equal(
    estimateVertexCostUsd("gemini-3.8-flash", {
...
test("estimateVertexCostUsd bills Google Search grounding per prompt", () => {
  assert.equal(
    estimateVertexCostUsd("gemini-3.8-flash", undefined, { searchGroundingRequests: 3 }),
    3 * SEARCH_GROUNDING_COST["gemini-3.8-flash"],
  );
  ...
  assert.equal(
    ...
    1.5 + SEARCH_GROUNDING_COST["gemini-3.8-flash"],
  );
});

test("grounding requests are ignored when absent or nonsensical", () => {
  assert.equal(estimateVertexCostUsd("gemini-3.8-flash", undefined), 0);
  assert.equal(
    estimateVertexCostUsd("gemini-3.8-flash", undefined, { searchGroundingRequests: -5 }),
    0,
  );
});
