...
17. **`sheet-write` `data:` as a YAML mapping** — The handler calls `JSON.parse` on the raw `data:` value, so it MUST be a single-quoted JSON string: `data: '[{"id": "{{prepared.id}}", "email": "{{auth.email}}"}]'`. A YAML mapping form (`data:\n  id: "..."\n  email: "..."`) is not accepted — the template engine can't interpolate into a non-string value and the node throws at runtime. The same rule applies to `sheet-update` and `sheet-delete` `data:` / `filter:`.
18. **Raw ISO timestamps in user-facing output** — `{{request.body.start}}` resolves to a machine timestamp like `2025-04-02T10:30:00.000Z`. Never embed it directly in an email body, dialog message, or JSON response that a human reads. Format it in a `script` node (e.g. `new Date(start).toLocaleString("ja-JP", { dateStyle: "long", timeStyle: "short", timeZone: "Asia/Tokyo" })`) and reference `{{prepared.displayRange}}` (or whatever field you named) from the `gmail-send` body. Storage columns like `sheet-write scheduled_at` should keep the raw ISO — formatting is only for what the user sees. See `references/sample-interview.md` for the full pattern.
19. **Protected page missing email + logout** — Pages guarded by `gemihub.auth.require()` MUST render the authenticated user's email and a logout button in a header. The Protected Page Template in `references/page-patterns.md` already wires `<span id="user-name">` and `<button id="logout-btn">` — copy the template VERBATIM; don't strip the header when customizing the body.
