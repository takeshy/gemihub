...
- Schema migration now runs **automatically** as a side effect of `update_drive_file` / `create_drive_file` whenever the path is `web/__gemihub/schema.md`. The file-write response carries a `schemaMigration` field with `created` / `updated` / `unchanged` lists (or `error`). The previous workflow required the LLM to explicitly call `migrate_spreadsheet_schema` after saving schema.md, which it routinely forgot — leaving sheets out of sync. The explicit tool is still available as a retry / dry-run path. SKILL.md and the interview sample reference were updated to describe the new flow.
