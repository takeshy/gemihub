...
さらに、Drive 上の Markdown 製ナレッジベース（概念・メトリクス・用語集・プレイブックなど）を **OKF（Open Knowledge Format）バンドル**としてチャットのナレッジに使えます。設定の RAG タブで OKF の親フォルダを指定しておき、どのバンドルを使うかはチャット入力欄の上のセレクタでチャットごとに選択します（詳細は [docs/references/OKF.md](./docs/references/OKF.md)）。
