...
ホーム画面はカスタマイズ可能なダッシュボードです。Markdown メモ、ファイル一覧、カードグリッド、編集可能なテーブル、カンバンボード、ワークフロー出力、Web ページ埋め込みといったウィジェットを、ドラッグ＆ドロップのグリッド上に自由に配置できます。編集モードに切り替えれば、ウィジェットの追加・設定・リサイズ・並べ替えが可能で、Undo / Redo にも対応。複数のダッシュボードを作成して切り替えたり、ホームに固定したりできます。カード／テーブル／カンバンウィジェットはフォルダ内の Markdown ファイルを読み込み（フィルタ／ソート／件数制限つき）、テーブルでは frontmatter をセル上で直接編集でき、カンバンカードはボードヘッダーから作成したり、ステータス列間のドラッグで Drive のファイルへ書き戻したりできます。ワークフローウィジェットは GemiHub のワークフローを実行し、その出力をカード・テーブル・Markdown・HTML として描画します（任意で自動更新）。各ダッシュボードは Drive 上の `.dashboard` ファイルとして保存され、レンダリング表示と生の YAML 表示を切り替えられます。
...
- **カスタマイズ可能なダッシュボード** — ホーム画面となるドラッグ＆ドロップのウィジェットグリッド。Markdown（インライン or Drive ファイル参照）、ファイル一覧、フォルダ内 Markdown を対象としたカードグリッド／テーブル／カンバン（新規カード作成、未該当ステータス列の表示切替、フィルタ／ソート／件数制限、frontmatter の編集を `.md` に書き戻し）、ワークフローを実行して出力を描画するウィジェット（カード／テーブル／Markdown／HTML、任意で自動更新）、Web 埋め込みに対応。複数ダッシュボード、編集モード、Undo/Redo、ホーム固定、レンダリング／生 YAML 表示切替
